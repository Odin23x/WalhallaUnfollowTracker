...
function Write-Log {
    ...
    try {
        if (Test-Path $LogFile) {
            ...
            if ($existing.Count -ge 400) {
                $trimmed = $existing[($existing.Count - 350)..($existing.Count - 1)]
...
function Save-Followers {
    ...
    Rename-Item -Path $tmp -NewName (Split-Path -Leaf $FollowersFile)
}
...
function Save-Events {
    ...
    Rename-Item -Path $tmp -NewName (Split-Path -Leaf $EventsFile)
}
...
function Run-Check {
    ...
    Set-State 'odin23x.twitch_unfollow_tracker_clean.state.status' 'Pruefe...'
...
function Handle-Message {
    ...
    switch ([string]$msg.type) {
        ...
        'closePlugin' {
            Write-Log 'Touch Portal requested plugin shutdown.'
            exit 0
        }
    }
}
